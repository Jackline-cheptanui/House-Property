...
class HomeController extends Controller
{
    ...
    public function searchwork(Request $request)
    {
        ...
        $rooms=property_rooms::with('getRoomfiles')->where('room_name','LIKE',"%{$search}%")->get();
    ...
    public function getallPropertie()
    {
        $properties = Properties::paginate(10);
   //dd($properties);
        return view('users.properties', [
            'property' => $properties
        ]);
    }
    public function edit($id)
    {
        ...
        $propertiestype = property_types::get();
        return view('users.edit', ['properties' => $properties,'getroomtypes'=>$propertiestype]);
    }
    public function updateproperty(Request $request, $id)
    {
       DB::beginTransaction();
        try {
            $permission = Properties::whereId($id)->first();
            $permission->property_description=$request->room_description;
            $permission->property_name=$request->room_name;
            $permission->property_location=$request->room_location;
            $permission->save();
            
           // dd($permission);
            DB::commit();
            return redirect()->back()->with('success','Poperty updated successfully.');
        } catch (\Throwable $th) {
            DB::rollback();
            return redirect()->route('users.edit',['permission' => $permission])->with('error',$th->getMessage());
        }
    }
    public function destroy($id)
    {
        ...
        try {
            ...
            Properties::whereId($id)->delete();
            
            DB::commit();
            return redirect()->back()->with('success','Poperty deleted successfully.');
        } catch (\Throwable $th) {
            DB::rollback();
