...
<body>
    <div id="app">
        ...
        <div class="input-group">
            <input type="text" id="searches"class="form-control bg-light border-0 small" placeholder="Search for..."
  ...
  crossorigin="anonymous"></script>
    <script>
      $(document).ready(function() {
        ...
        $.ajax({
                            url:"{{ route('rooms.searchwork') }}",
                            type:"POST",
                            data:searchterm,
                            success:function(response)
                            {
                              var apen=$('.listing-section');
                              let appending='';
                              //var data = JSON.parse(response);
                              $.each(response, function(k, v) {
                                alert(v.id);
                                var image ='';
                                $.each(v.getRoomfiles, function(k, m) {
                                image +=m.property_image;
                                });
                                appending +=  '<div class="product"><div class="image-box"><div class="images" id="image-1"><img   src="{{url('/storage/attachments/+image+')}}" alt="Image"/></div></div><div class="text-box"><h2 class="item">'+v.room_name+'</h2><h3 class="price">$4.99</h3><p class="description">'+v.room_description+'</p><button type="button" name="item-1-button" id="item-1-button">Book Room</button></div></div>';
                              });
                              apen.append(appending);
                            },
                            error:function(error)
                            {
                              swal("Thats great!", "Your Event Deleted!", "success", {
                                  icon: "danger",
                                });
                            },
                        });
